...
static avan_header_t *schedule = NULL;
...
void initialize_cs_system() {
  ...
  schedule = avan_create();
  if(schedule == NULL) {
    abort_error("Failed to initialize the Scheduler System (avan_create returned NULL).", __FILE__);
  }
}
...
void cs_exiting_process(int exit_code) {
  if(on_cpu) {
    int exit = 0;
    exit = avan_quit(schedule, on_cpu, exit_code);
    sprintf(g_status_msg, "Terminating PID %d, got back exit code %d from avan_quit\n", on_cpu->pid, exit);
    print_debug(g_status_msg);
    on_cpu = NULL;
  }
  else {
...
void cs_avan_process(process_data_t *proc) {
  process_node_t *proc_node = avan_new_process(proc->cmd, proc->pid, proc->priority_level, proc->is_critical);
  avan_insert(schedule, proc_node);
  print_avan_debug(schedule);
}

// Tells the schedule to terminate the process with the given exit code
void cs_avan_terminate(pid_t pid, int exit_code) {
  int last_state = -1;
  int exit = 0;
  ...
  if(on_cpu && on_cpu->pid == pid) {
    ...
    exit = avan_terminate(schedule, pid, exit_code);
    sprintf(g_status_msg, "Terminating PID %d, got back exit code %d from avan_terminate\n", pid, exit);
    print_debug(g_status_msg);
  }
  if(last_state == 1) {
    start_cs();
...
void print_schedule() {
  ...
  sprintf(g_status_msg, "...[Ready Queue - %d Processes]", avan_get_size(schedule->ready_queue));
  print_status(g_status_msg);
  print_avan_queue(schedule->ready_queue);
  sprintf(g_status_msg, "...[Suspended Queue - %d Processes]", avan_get_size(schedule->suspended_queue));
  print_status(g_status_msg);
  print_avan_queue(schedule->suspended_queue);
  sprintf(g_status_msg, "...[Terminated Queue - %d Processes]", avan_get_size(schedule->terminated_queue));
  print_status(g_status_msg);
  print_avan_queue(schedule->terminated_queue);
}

// Prints a single Scheduler Queue
void print_avan_queue(queue_header_t *queue) {
